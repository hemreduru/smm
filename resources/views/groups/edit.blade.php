...
@section('title', __('messages.edit_group') . ' - ' . $group->name)
...
@section('toolbar')
    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
        ...
        <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
            ...
            <li class="breadcrumb-item text-muted">{{ __('messages.edit_group') }}</li>
        </ul>
    </div>
@endsection
...
@section('content')
    <div class="card card-flush">
        <div class="card-header">
            <h3 class="card-title">{{ __('messages.edit_group') }}: {{ $group->name }}</h3>
        </div>
        <div class="card-body">
            <form action="{{ route('groups.update', $group) }}" method="POST">
                ...
                <div class="fv-row mb-7">
                    ...
                    <textarea class="form-control form-control-solid @error('description') is-invalid @enderror" 
                              ...
                              placeholder="Optional description for this group">{{ old('description', $group->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ route('groups.show', $group) }}" class="btn btn-light me-3">{{ __('messages.cancel') }}</a>
